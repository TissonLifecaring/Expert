...
@interface UzysAssetsPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UzysAssetsViewCellDelegate,MWPhotoBrowserDelegate>
//View
...
- (id)init
{
    self = [super initWithNibName:@"try" bundle:nil];
    if(self)
    {
...
- (void)setupCollectionView
{
    ...
    layout.itemSize                     = CGSizeMake([self itemWidthHeight], [self itemWidthHeight]);
    layout.sectionInset                 = UIEdgeInsetsMake(0, 0, 0, 0);
    layout.minimumInteritemSpacing      = 1.0;
    layout.minimumLineSpacing           = 5.0;
    
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 64, CGRectGetWidth(rect), CGRectGetHeight(rect) - 64 -48) collectionViewLayout:layout];
...
- (void)reloadData
{
    ...
    [self.btnDone setTitle:[NSString stringWithFormat:@"%lu",(unsigned long)self.selectAssets.count]
                  forState:UIControlStateNormal];
    [self showNoAssetsIfNeeded];
    
    [self updateBtnDoneColorWithSelectNumber:self.selectAssets.count];
}
...
- (void)setAssetsCountWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    [self updateBtnDoneColorWithSelectNumber:indexPaths.count];
}

- (void)updateBtnDoneColorWithSelectNumber:(NSInteger)selectedNumber
{
    if (selectedNumber<=0)
    {
    ...
    {
        self.btnDone.backgroundColor = UIColorFromRGB(0x3D8CD3);
    }
}
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation
{
    ...
    [self updateCollectionViewFrame];
    [self.groupPicker updateViewsFrame];
}
...
//    [self.collectionView reloadData]; 
    [self.collectionView setFrame:CGRectMake(0, 64, CGRectGetWidth(rect), CGRectGetHeight(rect) - 64 -48)];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL isSelectImage = NO;
    
    
    for (ALAsset *selectAsset in self.selectAssets)
    {
        NSString *selectUrl = [NSString stringWithFormat:@"%@",selectAsset.defaultRepresentation.url];
        NSString *assetUrl = [NSString stringWithFormat:@"%@",asset.defaultRepresentation.url];
        
        if ([selectUrl isEqualToString:assetUrl])
        {
            isSelectImage = YES;
        }
    }
...
#pragma mark - UzysAssetsViewCell Delegate
- (void)uzysAssetsViewCell:(UzysAssetsViewCell *)cell didClickSmallButton:(UIButton *)button
{
    ...
    {
        ALAsset *selectAsset = self.selectAssets[i];
    ...
    if (isSelected)
    {
        if (self.selectAssets.count > selectIndex)
        {
            [self.selectAssets removeObjectAtIndex:selectIndex];
        }
    }
    else
    {
    ...
    [self setAssetsCountWithSelectedIndexPaths:self.selectAssets];
}

- (BOOL)uzysAssetsViewCell:(UzysAssetsViewCell *)cell shouldSelectItemWithButton:(UIButton *)button
{
    return (self.selectAssets.count < self.maximumNumberOfSelection);
}
...
- (void)finishPickingAssets
{
    ...
    if([self.selectAssets count]>0)
    {
        ...
        if([picker.delegate respondsToSelector:@selector(UzysAssetsPickerController:didFinishPickingAssets:)])
            [picker.delegate UzysAssetsPickerController:picker didFinishPickingAssets:self.selectAssets];
...
- (void)setTitle:(NSString *)title
{
    ...
    [self.btnTitle setImageEdgeInsets:UIEdgeInsetsMake(0, self.btnTitle.titleLabel.frame.origin.x +self.btnTitle.titleLabel.frame.size.width + 15, 0, 0)];
